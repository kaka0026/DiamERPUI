...
import { SubcategoryMastService } from "../../Services/DesignMast/subcategory-mast.service";
...
@Component({
  selector: 'app-subcategory-mast',
  templateUrl: './subcategory-mast.component.html',
  styleUrls: ['./subcategory-mast.component.css']
})
export class SubcategoryMastComponent implements OnInit {
  ...
  constructor(
    ...
    private SubcategoryMastSer: SubcategoryMastService,
      ...
      },
      {
        ...
        cellRenderer: function (params) {
          ...
          if (params.data.IsActive == true) {
            ...
            if (_delete == true) {
              html += '<div style="color: #f44336;" class="delete"><button title="click here for delete record" type="button" class="la la-trash" value="Delete" data-action-type="DeleteUser"></button></div> '
            }
  ...
  ngOnInit(): void {
    this.spinner.show();
    this.SubcategoryMastSer.CategoryDDL({}).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      this.NAME = eve.data.Sub_Category_Name;
      this.CAT = eve.data.Category_Name;
      this.SCODE = eve.data.Sub_Category_Id;
    ...
    if (actionType == "DeleteUser") {
      Swal.fire({
        title: "Are you sure you want to delete " + eve.data.Sub_Category_Name + "?",
        icon: "warning",
        cancelButtonText: "No",
        ...
        if (result.value) {
          let resultObj = {
            USERID: this.decodedTkn.UserId,
            Sub_Category_Id: eve.data.Sub_Category_Id
          };
          this.SubcategoryMastSer.SubCategoryDelete(resultObj).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
    ...
    if (actionType == "TrackUser") {
      const PRF = this.dialog.open(TrackMastComponent, { width: '80%', data: { O_id: eve.data.Sub_Category_Id, PageName: 'SubCategoryMast.aspx' } })
  ...
  LoadGridData() {
    this.spinner.show();
    this.SubcategoryMastSer.SubCategoryFill({}).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  Save() {
    ...
    if (this.NAME.trim() == "") {
      this.toastr.clear();
      this.toastr.warning("Enter Sub Category");
      return;
    }
    if (this.CAT == "" || this.Category.value == null || this.Flag == false) {
      this.toastr.clear();
    ...
    this.spinner.show();
    let ResulObj = {
      Sub_Category_Id: this.SCODE == 0 ? 0 : this.SCODE,
    ...
    this.SubcategoryMastSer.SubCategorySave(ResulObj).subscribe(Result => {
      try {
        if (Result.Success == 1) {
          this.Clear();
          this.toastr.clear();
