...
import { ClientViewFormComponent } from "../client-view-form/client-view-form.component"
...
export class ClientComponent implements OnInit {
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      localStorage.setItem('ClientID', eve.data.ClientID)
      this.DashboardServ.setData("ClientUpdate (" + eve.data.ClientID + ")");
    }
    if (actionType == "DeleteUser") {
      Swal.fire({
        ...
        this.spinner.show();
        if (result.value) {
          this.ClientServiceSer.ClientDelete({ ClientID: eve.data.ClientID }).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
    ...
    if (actionType == "ViewUser") {
      const PRF = this.dialog.open(ClientViewFormComponent, { width: '80%', data: { CLIENTID: eve.data.ClientID, NAME: eve.data.Name } })
  ...
  add() {
    this.DashboardServ.setData("Clientnew");
  }
}
