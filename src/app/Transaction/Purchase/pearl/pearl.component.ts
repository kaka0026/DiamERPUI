...
export class PearlComponent implements OnInit {
  ...
  DrillStatus: FormControl;
  filteredDrillStatus: Observable<any[]>;
  DrillStatusCode: any = "";
  DrillStatusArr: Lot1[] = [];
  DrillStatusName: any = '';

  GoodsStatus: FormControl;
  filteredGoodsStatus: Observable<any[]>;
  GoodsStatusCode: any = "";
  GoodsStatusArr: Lot1[] = [];
  GoodsStatusName: any = '';
  ...
  constructor(
    ...
    private _mdr: MatDialogRef<PearlComponent>,
    ...
    this.DrillStatus = new FormControl();
    this.GoodsStatus = new FormControl();
    this.MemoNo = valdata.MemoNo;

    this.DrillStatusArr = [{ name: 'UNDRILLED', code: 'UNDRILLED' }, { name: 'HALFDRILLED', code: 'HALFDRILLED' }, { name: 'FULLDRILLED', code: 'FULLDRILLED' }]
    this.filteredDrillStatus = this.DrillStatus.valueChanges.pipe(
      startWith(""),
      map(lot => (lot ? this.filteredDrillStatuss(lot) : this.DrillStatusArr.slice()))
    );

    this.GoodsStatusArr = [{ name: 'LOOSE', code: 'LOOSE' }, { name: 'STRING', code: 'STRING' }, { name: 'TASSLE', code: 'TASSLE' }, { name: 'PAIR', code: 'PAIR' }]
    this.filteredGoodsStatus = this.GoodsStatus.valueChanges.pipe(
      startWith(""),
      map(lot => (lot ? this.filteredGoodsStatuss(lot) : this.GoodsStatusArr.slice()))
    );
              ...
              this.PurchaseSer.PurchaseLFillModel(
                ...
                try {
                  if (Result.Success == 1) {
                    ...
                    this.DrillStatusCode = Result.Data[0].DRILLSTATUS;
                    this.SelectDrillStatusOption()
                    this.GoodsStatusCode = Result.Data[0].GOODSSTATUS;
                    this.SelectGoodsStatusOption()
                    ...
                    this.DrillStatus.disable();
                    this.DrillStatus.disabled

                    this.GoodsStatus.disable();
                    this.GoodsStatus.disabled
  ...
  SelectDrillStatusOption() {
    if (this.DrillStatus.value != "") {
    }
  }
  SelectGoodsStatusOption() {
    if (this.GoodsStatus.value != "") {
    }
  }
  Close() {
    this._mdr.close();
  ...
  filteredDrillStatuss(name: string) {
    const filterValue = name.toLowerCase();
    const a = this.DrillStatusArr.filter(option => option.name.toLowerCase().includes(filterValue));
    return a;
  }
  filteredGoodsStatuss(name: string) {
    const filterValue = name.toLowerCase();
    const a = this.GoodsStatusArr.filter(option => option.name.toLowerCase().includes(filterValue));
    return a;
  }
  DISALEMEMO(e: any) {
    if (e == '1') {
  ...
  Save() {
    ...
    let resultobj = {
      ...
      DRILLSTATUS: this.DrillStatus.value.toString(),
      GOODSSTATUS: this.GoodsStatus.value.toString(),
      LAB: '',
      CERTNO: '',
      COLORSCALE: '',
      COLORSHADE: '',
