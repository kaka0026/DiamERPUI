...
export class PurchaseEditComponent implements OnInit {
  ...
  LoadGrid() {
    this.spinner.show();
    this.PurchaseServ.PurchaseHFill({ TrnNo: '', ID: 0 }).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      localStorage.setItem('PurchaseTrnNo', eve.data.TrnNo);
      localStorage.removeItem('PurchaseID');
      localStorage.setItem('PurchaseID', eve.data.ID);
      this.DashboardServ.setData("PurchaseUpdate (" + eve.data.TrnNo + ")");
    }
    if (actionType == "DeleteUser") {
      Swal.fire({
        ...
        this.spinner.show();
        if (result.value) {
          this.PurchaseServ.PurchaseHDelete({ TrnNo: eve.data.TrnNo }).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
                this.LoadGrid();
  ...
  add() {
    this.DashboardServ.setData("PurchaseNew");
  }
