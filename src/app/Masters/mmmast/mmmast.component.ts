...
@Component({
  selector: 'app-mmmast',
  templateUrl: './mmmast.component.html',
  styleUrls: ['./mmmast.component.css']
})
export class MMMastComponent implements OnInit {
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      this.CODE = eve.data.MM_SORTNAME;
      this.DESC = eve.data.MM_DESC;
      this.SCODE = eve.data.MM_CODE;
      this.Material = new FormControl(eve.data.Material ? eve.data.Material.split(",") : "");
      this.materialchange();
      // this.Shape =  new FormControl(eve.data.Shape ? eve.data.Shape.split(",") : "");
      this.Shape.setValue(eve.data.Shape ? eve.data.Shape.split(",") : "");
      this.chkIsActive = eve.data.IsActive;
      this.FROM = eve.data.F_MM;
      this.TO = eve.data.T_MM;
      $("#code").focus();
    }
    if (actionType == "DeleteUser") {
      Swal.fire({
        title: "Are you sure you want to delete " + eve.data.MM_CODE + "?",
        icon: "warning",
        cancelButtonText: "No",
        ...
        if (result.value) {
          let resultObj = {
            USERID: this.decodedTkn.UserId,
            MM_CODE: eve.data.MM_CODE
          }
          this.MMMastSer.MMMastDelete(resultObj).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
    ...
    if (actionType == "TrackUser") {
      const PRF = this.dialog.open(TrackMastComponent, { width: '80%', data: { O_id: eve.data.MM_CODE, PageName: 'MMMast.aspx' } })
  ...
  LoadGridData() {
    this.spinner.show();
    this.MMMastSer.MMMastFill({}).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  Save() {
    ...
    if (this.Shape.value == null || this.Shape.value.toString() == "") {
      this.toastr.clear();
    ...
    this.spinner.show();
    let ResulObj = {
      MM_CODE: this.SCODE == 0 ? 0 : this.SCODE,
      MM_DESC: this.DESC,
      F_MM: this.FROM,
      T_MM: this.TO,
      USERID: this.DESC,
      MM_SORTNAME: this.CODE,
      Material: this.Material.value.toString(),
      Shape: this.Shape.value.toString(),
      IsActive: this.chkIsActive
    };
    this.MMMastSer.MMMastSave(ResulObj).subscribe(Result => {
      try {
        if (Result.Success == 1) {
          this.toastr.clear();
  ...
  CheckCode() {
    if (this.CODE.trim() != "") {
      this.MMMastSer.MMMastCheck({ MM_SORTNAME: this.CODE }).subscribe(Result => {
        try {
          if (Result.Success == 1) {
            Swal.fire({
