...
import { InclusionCatService } from "../../Services/Masters/inclusion-cat.service";
...
@Component({
  selector: 'app-inclusion-cat',
  templateUrl: './inclusion-cat.component.html',
  styleUrls: ['./inclusion-cat.component.css']
})
export class InclusionCatComponent implements OnInit {
  ...
  constructor(
    ...
    private InclusionCatSer: InclusionCatService,
    ...
    let ObjResult = {
      ...
      PAGENAME: "InclusionCategoryMast.aspx"
    }
    let _edit;
    let _delete;
    let _track;
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      this.IncCODE = eve.data.INC_CAT_CODE;
      this.CODE = eve.data.INC_CAT_SORTNAME;
      this.NAME = eve.data.INC_CAT_NAME;
      this.chkIsActive = eve.data.IsActive;
      $("#code").focus();
    }
    if (actionType == "DeleteUser") {
      Swal.fire({
        title: "Are you sure you want to delete " + eve.data.INC_CAT_SORTNAME + "?",
        icon: "warning",
        cancelButtonText: "No",
        ...
        if (result.value) {
          let resultObj = {
            USERID: this.decodedTkn.UserId,
            INC_CAT_CODE: eve.data.INC_CAT_CODE
          }
          this.InclusionCatSer.IncCatDelete(resultObj).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
    ...
    if (actionType == "TrackUser") {
      const PRF = this.dialog.open(TrackMastComponent, { width: '80%', data: { O_id: eve.data.INC_CAT_CODE, PageName: 'InclusionCategoryMast.aspx' } })
  ...
  LoadGridData() {
    this.spinner.show();
    this.InclusionCatSer.IncCatFill({}).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  Save() {
    ...
    let ResulObj = {
      USERID: this.decodedTkn.UserId,
      INC_CAT_NAME: this.NAME,
      INC_CAT_CODE: this.IncCODE == 0 ? "" : this.IncCODE,
      INC_CAT_SORTNAME: this.CODE,
      IsActive: this.chkIsActive
    };
    this.InclusionCatSer.IncCatSave(ResulObj).subscribe(Result => {
      try {
        if (Result.Success == 1) {
          this.toastr.clear();
  ...
  CheckCode() {
    if (this.CODE.trim() != "") {
      this.InclusionCatSer.IncCatCheck({ INC_SORTNAME: this.CODE }).subscribe(Result => {
        try {
          if (Result.Success == 1) {
            Swal.fire({
