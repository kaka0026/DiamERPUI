...
@Component({
  selector: 'app-clarity-mast',
  templateUrl: './clarity-mast.component.html',
  styleUrls: ['./clarity-mast.component.css']
})
export class ClarityMastComponent implements OnInit {
  ...
  fillMaterial() {
    this.spinner.show();
    this.CalrityMastSer.MaterialMastDDL({}).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      this.CODE = eve.data.Q_SortName;
      this.NAME = eve.data.Q_Name;
      this.SCODE = eve.data.Q_Code;
    ...
    if (actionType == "DeleteUser") {
      Swal.fire({
        title: "Are you sure you want to delete " + eve.data.Q_Name + "?",
        icon: "warning",
        cancelButtonText: "No",
        ...
        if (result.value) {
          ...
          this.CalrityMastSer.CalMastDelete(resultObj).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
    ...
    if (actionType == "TrackUser") {
      const PRF = this.dialog.open(TrackMastComponent, { width: '80%', data: { O_id: eve.data.Q_Code, PageName: 'ClarityMast.aspx' } })
  ...
  Save() {
    ...
    this.spinner.show();
    let ResulObj = {
      Q_Code: this.SCODE == 0 ? 0 : this.SCODE,
      Q_Name: this.NAME,
      Q_SortName: this.CODE,
      Material_CODE: this.Material.value.toString(),
      USERID: this.decodedTkn.UserId,
      IsActive: this.chkIsActive
    };
    this.CalrityMastSer.CalMastSave(ResulObj).subscribe(Result => {
      try {
        if (Result.Success == 1) {
          this.Clear();
          this.toastr.clear();
  ...
  CheckCode() {
    if (this.CODE.trim() != "") {
      this.CalrityMastSer.CalMastCheck({ Q_SortName: this.CODE }).subscribe(Result => {
        try {
          if (Result.Success == 1) {
            Swal.fire({
