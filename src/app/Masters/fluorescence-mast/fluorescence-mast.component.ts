...
@Component({
  selector: 'app-fluorescence-mast',
  templateUrl: './fluorescence-mast.component.html',
  styleUrls: ['./fluorescence-mast.component.css']
})
export class FluorescenceMastComponent implements OnInit {
  ...
  constructor(
    ...
    let ObjResult = {
      ...
      PAGENAME: "FluorescenceMast.aspx"
    }
    let _edit;
    let _delete;
    let _track;
  ...
  fillMaterial() {
    this.spinner.show();
    this.FluorescenceMastSer.MaterialMastDDL({}).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      this.CODE = eve.data.FLO_SORTNAME;
      this.DESC = eve.data.FLO_DESC;
      this.SCODE = eve.data.FLO_CODE;
    ...
    if (actionType == "DeleteUser") {
      Swal.fire({
        title: "Are you sure you want to delete " + eve.data.FLO_DESC + "?",
        icon: "warning",
        cancelButtonText: "No",
        ...
        if (result.value) {
          let resultObj = {
            USERID: this.decodedTkn.UserId,
            FLO_CODE: eve.data.FLO_CODE
          };
          this.FluorescenceMastSer.FloMastDelete(resultObj).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
    ...
    if (actionType == "TrackUser") {
      const PRF = this.dialog.open(TrackMastComponent, { width: '80%', data: { O_id: eve.data.FLO_CODE, PageName: 'FluorescenceMast.aspx' } })
  ...
  Save() {
    if (this.DESC.trim() == "") {
      this.toastr.clear();
      this.toastr.warning("Enter DESC");
      return;
    }
    if (this.CODE.trim() == "") {
      this.toastr.clear();
    ...
    this.spinner.show();
    let ResulObj = {
      FLO_CODE: this.SCODE == 0 ? 0 : this.SCODE,
      FLO_DESC: this.DESC,
      FLO_SORTNAME: this.CODE,
      Material_CODE: this.Material.value.toString(),
      USERID: this.decodedTkn.UserId,
      IsActive: this.chkIsActive
    };
    this.FluorescenceMastSer.FloMastSave(ResulObj).subscribe(Result => {
      try {
        if (Result.Success == 1) {
          this.Clear();
          this.toastr.clear();
  ...
  CheckCode() {
    if (this.CODE.trim() != "") {
      this.FluorescenceMastSer.FloMastCheck({ FLO_SORTNAME: this.CODE }).subscribe(Result => {
        try {
          if (Result.Success == 1) {
            Swal.fire({
