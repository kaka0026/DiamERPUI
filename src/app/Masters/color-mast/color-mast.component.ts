...
@Component({
  selector: 'app-color-mast',
  templateUrl: './color-mast.component.html',
  styleUrls: ['./color-mast.component.css']
})
export class ColorMastComponent implements OnInit {
  ...
  fillMaterial() {
    this.spinner.show();
    this.ColMastSer.MaterialMastDDL({}).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      this.CODE = eve.data.C_SortName;
      this.NAME = eve.data.C_Name;
      this.SCODE = eve.data.C_Code;
    ...
    if (actionType == "DeleteUser") {
      Swal.fire({
        title: "Are you sure you want to delete " + eve.data.C_Name + "?",
        icon: "warning",
        cancelButtonText: "No",
        ...
        if (result.value) {
          ...
          this.ColMastSer.ColMastDelete(resultObj).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
    ...
    if (actionType == "TrackUser") {
      const PRF = this.dialog.open(TrackMastComponent, { width: '80%', data: { O_id: eve.data.C_Code, PageName: 'ColorMast.aspx' } })
  ...
  Save() {
    ...
    this.spinner.show();
    let ResulObj = {
      C_Code: this.SCODE == 0 ? 0 : this.SCODE,
      C_Name: this.NAME,
      C_SortName: this.CODE,
      Material_CODE: this.Material.value.toString(),
      USERID: this.decodedTkn.UserId,
      IsActive: this.chkIsActive
    };
    this.ColMastSer.ColMastSave(ResulObj).subscribe(Result => {
      try {
        if (Result.Success == 1) {
          this.Clear();
          this.toastr.clear();
  ...
  CheckCode() {
    if (this.CODE.trim() != "") {
      this.ColMastSer.ColMastCheck({ C_SortName: this.CODE }).subscribe(Result => {
        try {
          if (Result.Success == 1) {
            Swal.fire({
