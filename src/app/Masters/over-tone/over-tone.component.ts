...
import { OverToneService } from 'src/app/Services/Masters/over-tone.service';
...
@Component({
  selector: 'app-over-tone',
  templateUrl: './over-tone.component.html',
  styleUrls: ['./over-tone.component.css']
})
export class OverToneComponent implements OnInit {
  ...
  constructor(
    ...
    private OverToneSer: OverToneService,
  ...
  fillMaterial() {
    this.spinner.show();
    this.OverToneSer.MaterialMastDDL({}).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      this.CODE = eve.data.OV_SortName;
      this.NAME = eve.data.OV_Name;
      this.SCODE = eve.data.OV_Code;
    ...
    if (actionType == "DeleteUser") {
      Swal.fire({
        title: "Are you sure you want to delete " + eve.data.OV_Name + "?",
        icon: "warning",
        cancelButtonText: "No",
        ...
        if (result.value) {
          let resultObj = {
            USERID: this.decodedTkn.UserId,
            OV_Code: eve.data.OV_Code
          };
          this.OverToneSer.OverToneMastDelete(resultObj).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
    ...
    if (actionType == "TrackUser") {
      const PRF = this.dialog.open(TrackMastComponent, { width: '80%', data: { O_id: eve.data.OV_Code, PageName: 'OverToneMast.aspx' } })
  ...
  LoadGridData() {
    this.spinner.show();
    this.OverToneSer.OverToneMastFill({}).subscribe(Result => {
      try {
        if (Result.Success == 1) {
  ...
  Save() {
    ...
    this.spinner.show();
    let ResulObj = {
      OV_Code: this.SCODE == 0 ? 0 : this.SCODE,
      OV_Name: this.NAME,
      OV_SortName: this.CODE,
      Material_CODE: this.Material.value.toString(),
      USERID: this.decodedTkn.UserId,
      IsActive: this.chkIsActive
    };
    this.OverToneSer.OverToneMastSave(ResulObj).subscribe(Result => {
      try {
        if (Result.Success == 1) {
          this.Clear();
          this.toastr.clear();
  ...
  CheckCode() {
    if (this.CODE.trim() != "") {
      this.OverToneSer.OverToneMastCheck({ OV_SortName: this.CODE }).subscribe(Result => {
        try {
          if (Result.Success == 1) {
            Swal.fire({
