...
@Component({
  selector: 'app-location-mast',
  templateUrl: './location-mast.component.html',
  styleUrls: ['./location-mast.component.css']
})
export class LocationMastComponent implements OnInit {
  ...
  onGridRowClicked(eve: any) {
    ...
    if (actionType == "UpdateUser") {
      ...
      this.CODE = eve.data.Location_Code;
      this.NAME = eve.data.Location_Name;
      this.SCODE = eve.data.Location_Id;
      this.chkIsActive = eve.data.IsActive;
      $("#code").focus();
    }
    if (actionType == "DeleteUser") {
      Swal.fire({
        title: "Are you sure you want to delete " + eve.data.Location_Name + "?",
        icon: "warning",
        cancelButtonText: "No",
        ...
        if (result.value) {
          let resultObj = {
            USERID: this.decodedTkn.UserId,
            Location_Id: eve.data.Location_Id
          };
          this.LocationMastSer.LocationDelete(resultObj).subscribe(Result => {
            try {
              if (Result.Success == 1) {
                this.toastr.clear();
    ...
    if (actionType == "TrackUser") {
      const PRF = this.dialog.open(TrackMastComponent, { width: '80%', data: { O_id: eve.data.Location_Id, PageName: 'LocationMast.aspx' } })
  ...
  Save() {
    ...
    this.spinner.show();
    let ResulObj = {
      Location_Id: this.SCODE == 0 ? 0 : this.SCODE,
      Location_Name: this.NAME,
      Location_Code: this.CODE,
      USERID: this.decodedTkn.UserId,
      IsActive: this.chkIsActive
    };
    this.LocationMastSer.LocationSave(ResulObj).subscribe(Result => {
      try {
        if (Result.Success == 1) {
          this.Clear();
          this.toastr.clear();
  ...
  CheckCode() {
    if (this.CODE.trim() != "") {
      this.LocationMastSer.LocationCheck({ Location_Code: this.CODE }).subscribe(Result => {
        try {
          if (Result.Success == 1) {
            Swal.fire({
